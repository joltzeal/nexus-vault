...
export type ParsedResourceFileType =
  | "unknown"
  | "video"
  | "text"
  | "image"
...
export function parseResourceInput(input: ResourceInputParserInput): ParsedResourceInput {
...
export function parseEd2kLink(url: string): ParsedEd2kLink | null {
  ...
  if (!fileName || !Number.isFinite(fileSize) || fileSize < 0 || !hash) return null
...
export function parseCloudDriveLink(url: string): ParsedCloudDriveLink | null {
  const parsedUrl = parseHttpUrl(url)
  if (!parsedUrl) return null
  ...
  if (host === "pan.baidu.com") {
    return {
      ...
      password: getCloudDrivePassword(parsedUrl),
      shareId: getCloudDriveShareId(parsedUrl),
    }
  }
...
function defaultResourceTitle(type: ResourceType) {
  ...
  if (type === "baidu_pan") return "百度网盘"
  if (type === "quark_pan") return "夸克网盘"
...
function createParsedResourceInput(
  ...
  return {
    type,
    url: input.url.trim(),
    title: input.title?.trim() || defaultResourceTitle(type),
    metadata,
  }
}
...
export const magnetInputParser: ResourceInputParser = {
  name: "magnet",
  supports(input) {
    return input.type === "magnet" || input.url.trim().toLowerCase().startsWith("magnet:?")
  },
  parse(input) {
    ...
    return {
      type: "magnet",
      url: parsed.infoHash,
      title: input.title?.trim() || parsed.displayName || defaultResourceTitle("magnet"),
      metadata: {
...
export const ed2kInputParser: ResourceInputParser = {
  ...
  parse(input) {
    ...
    return {
      ...
      url: input.url.trim(),
      title: input.title?.trim() || parsed?.fileName || defaultResourceTitle(input.type ?? "other"),
      metadata: {
        protocol: "ed2k",
        ...(parsed
          ? {
...
export const httpInputParser: ResourceInputParser = {
  ...
  supports(input) {
    return (
      input.type === "http" ||
      input.type === "twitter" ||
      input.type === "baidu_pan" ||
      input.type === "quark_pan" ||
  ...
  parse(input) {
    ...
    if (input.type === "twitter" || twitter) {
      return {
        ...
        title: input.title?.trim() || defaultResourceTitle("twitter"),
        metadata: {
          ...(twitter
            ? {
    ...
    const cloudDrive = parseCloudDriveLink(input.url)
    if (
      input.type === "baidu_pan" ||
      input.type === "quark_pan" ||
      cloudDrive
    ) {
      const type = input.type === "baidu_pan" || input.type === "quark_pan"
        ? input.type
        : cloudDrive?.provider ?? "http"

      if (type === "baidu_pan" || type === "quark_pan") {
        return {
          type,
    ...
    return {
      ...
      title: input.title?.trim() || defaultResourceTitle("http"),
      metadata: {
        ...(http ? { host: http.host } : {}),
      },
    }
...
export function getResourceInputParser(input: Pick<ResourceInputParserInput, "type" | "url">) {
  return (
    resourceInputParsers.find((parser) => parser.supports(input)) ?? fallbackInputParser
  )
}
...
function normalizeBase64(value: string) {
  ...
  return padding === 0 ? decodedValue : decodedValue.padEnd(decodedValue.length + (4 - padding), "=")
}
...
function getCloudDrivePassword(url: URL) {
  return (
