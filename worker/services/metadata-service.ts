...
async function persistMagnetScreenshot(
  ...
  if (!response.ok) throw new Error(`Whatslink screenshot failed with HTTP ${response.status}.`)
  const image = await response.arrayBuffer()
  if (image.byteLength === 0) throw new Error("Whatslink screenshot is empty.")
