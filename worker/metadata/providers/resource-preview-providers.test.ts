...
test("Twitter provider uses guest scraper after empty syndication data", async (t) => {
  t.mock.method(globalThis, "fetch", async (input: RequestInfo | URL) => {
    const url = getRequestUrl(input)
    if (url.includes("cdn.syndication.twimg.com")) return jsonResponse({})
    throw new Error(`Unexpected request: ${url}`)
  })
  t.mock.method(
    Scraper.prototype,
    "getTweet",
    async () => createScraperTweet("Guest scraper post", "example"),
  )
  t.mock.method(Scraper.prototype, "getProfile", async () => {
    throw new DOMException("Profile request timed out", "TimeoutError")
  })

  const result = await twitterMetadataProvider.resolve(twitterResource)

  assert.equal(result.provider, "twitter-scraper")
  assert.equal(result.status, "completed")
  assert.equal(result.data.description, "Guest scraper post")
})

test("Twitter provider retries scraper with a cookie after guest has no result", async (t) => {
  let getTweetCalls = 0
  ...
  t.mock.method(Scraper.prototype, "setCookies", async () => {
    setCookieCalls += 1
  })
  t.mock.method(Scraper.prototype, "getTweet", async () => {
    getTweetCalls += 1
    return getTweetCalls === 1
      ? null
      : createScraperTweet("Cookie-authenticated scraper post")
  })

  const result = await twitterMetadataProvider.resolve(twitterResource, {
    twitterCookieString: "auth_token=test; ct0=test",
  })

  assert.equal(result.provider, "twitter-scraper")
  assert.equal(result.status, "completed")
  assert.equal(result.data.description, "Cookie-authenticated scraper post")
  assert.equal(getTweetCalls, 2)
  assert.equal(setCookieCalls, 1)
})

test("Twitter provider falls back to oEmbed when syndication is unavailable", async (t) => {
  t.mock.method(globalThis, "fetch", async (input: RequestInfo | URL) => {
    const url = getRequestUrl(input)
    if (url.includes("cdn.syndication.twimg.com")) {
    ...
    if (url.includes("publish.twitter.com/oembed")) {
      return jsonResponse({
        ...
        html: "<blockquote><p>Public post</p></blockquote>",
        url: twitterResource.url,
      })
    }
    throw new Error(`Unexpected request: ${url}`)
  })
  ...
  assert.equal(result.provider, "twitter-oembed")
  assert.equal(result.status, "completed")
  assert.equal(result.data.description, "Public post")
})
...
function createScraperTweet(text: string, username?: string) {
  return {
    id: "2085673727166165147",
    hashtags: [],
    likes: 1,
    mentions: [],
    name: "Example User",
