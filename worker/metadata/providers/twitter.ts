...
export const twitterMetadataProvider: MetadataProvider = {
  ...
  async resolve(resource, options) {
    ...
    const guestScraped = await fetchTweetWithScraper(parsed.tweetId)
    if (guestScraped.data) {
      return createScrapedTweetMetadata(
        baseMetadata,
        guestScraped.data,
        parsed.tweetId,
        parsed.username,
    ...
    if (guestScraped.retryableError) {
      retryableErrors.push(guestScraped.retryableError)
    }

    if (options?.twitterCookieString) {
      const authenticatedScraped = await fetchTweetWithScraper(
        parsed.tweetId,
        options.twitterCookieString,
      )
      if (authenticatedScraped.data) {
        return createScrapedTweetMetadata(
          baseMetadata,
          authenticatedScraped.data,
          parsed.tweetId,
          parsed.username,
        )
      }
      if (authenticatedScraped.retryableError) {
        retryableErrors.push(authenticatedScraped.retryableError)
      }
    }
...
async function fetchSyndicatedTweet(tweetId: string) {
  try {
    ...
    console.info("X syndication response", {
      tweetId,
      tweet,
    })
    return {
    ...
    console.warn("X syndication unavailable; falling back to twitter-scraper", {
      tweetId,
...
function createScrapedTweetMetadata(
  baseMetadata: ReturnType<typeof createBaseResourceMetadata>,
  scraped: { profile?: ScraperProfile; tweet: ScraperTweet },
  tweetId: string,
  username?: string,
) {
  ...
  return createResolvedTweetMetadata(baseMetadata, {
    media: getScrapedTweetMedia(scraped.tweet),
    metrics: {
      bookmarks: scraped.tweet.bookmarkCount,
      likes: scraped.tweet.likes,
      replies: scraped.tweet.replies,
      reposts: scraped.tweet.retweets,
      views: scraped.tweet.views,
    },
...
async function fetchTweetWithScraper(
  tweetId: string,
  cookieString?: string,
  ...
  try {
    ...
    if (!tweet) return {}
    console.info("Twitter scraper response", {
    ...
    return {
      retryableError:
        error instanceof RetryableMetadataError
          ? error.message
          : getRetryableTwitterError(
              error,
              cookieString ? "Cookie-authenticated X request" : "Guest X request",
            ),
    }
  }
}
