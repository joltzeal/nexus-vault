...
>(function AnimatedSidebarRail(
  ...
  return (
    <button
      ...
      className={cn(
        "absolute inset-y-0 z-20 hidden w-4 -translate-x-1/2 outline-none md:block group-data-[state=collapsed]/sidebar:hidden",
...
>(function AnimatedSidebarMenu(
  ...
  return (
    <SharedLayoutBg
      ...
      as="ul"
      inset={0}
      pillClassName="rounded-none bg-muted/70"
      pillContainerClassName={
        panel.collapsed ? "!inset-0" : "inset-y-auto top-0 h-9"
      }
      data-slot="sidebar-menu"
      className={cn(
        "flex w-full min-w-0 list-none flex-col gap-0.5",
        panel.collapsed && "items-center",
        className,
      )}
    >
      {children}
...
export function AnimatedSidebarMenuSubButton({
  ...
  const interactiveClassName = cn(
    "flex min-h-8 w-full min-w-0 items-center gap-2 rounded-none px-2 text-left text-xs outline-none",
...
export function AnimatedSidebarMenuButton({
    ...
    <>
      {isActive ? (
        <motion.span
          ...
          className="absolute inset-0 rounded-none border border-primary bg-primary/10"
        />
      ) : null}
      {icon ? (
        <span
      ...
      {ariaExpanded !== undefined ? (
        <motion.span
          ...
          className={cn(
            ...
            panel.collapsed && "pointer-events-none !size-0 overflow-hidden",
          )}
        >
  ...
  const interactiveClassName = cn(
    "relative flex min-h-9 w-full min-w-0 items-center gap-2.5 overflow-hidden rounded-none border border-transparent px-3 text-left text-sm font-medium outline-none",
    panel.collapsed
      ? "text-fg-muted transition-colors hover:border-transparent hover:bg-ink-800 hover:text-fg"
      : "text-fg-muted transition-colors hover:border-line-soft hover:bg-ink-800 hover:text-fg",
    "focus-visible:border-primary focus-visible:bg-ink-800 focus-visible:ring-2 focus-visible:ring-primary",
