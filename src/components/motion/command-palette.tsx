...
function getFuzzyMatchIndexes(needle: string, hay: string) {
  if (!needle) return new Set<number>();
  const query = Array.from(needle.toLocaleLowerCase());
  const characters = Array.from(hay);
  const matches = new Set<number>();
  let queryIndex = 0;

  for (const [index, character] of characters.entries()) {
    if (character.toLocaleLowerCase() !== query[queryIndex]) continue;
    matches.add(index);
    queryIndex += 1;
    if (queryIndex === query.length) return matches;
  }
...
function HighlightedText({
  value,
  query,
  emphasizeWholeValue = false,
}: {
  value: string;
  query: string;
  emphasizeWholeValue?: boolean;
}) {
  const matches = getFuzzyMatchIndexes(query, value);
  const characters = Array.from(value);
  const hasDirectMatch = matches.size > 0;

  return characters.map((character, index) =>
    hasDirectMatch && !matches.has(index) || !hasDirectMatch && !emphasizeWholeValue ? (
      character
    ) : (
      <mark
        className="bg-transparent text-primary underline decoration-primary/60 underline-offset-2"
        key={`${character}-${index}`}
      >
        {character}
      </mark>
                            ...
                            >
                              ...
                              <span className="relative z-10 min-w-0 flex-1">
                                <span className="block truncate">
                                  <HighlightedText
                                    emphasizeWholeValue={keywordMatch}
                                    query={query}
                                    value={it.label}
                                  />
                                </span>
