...
import { authPluginExtensions } from "./plugin-extensions"
...
export function Auth({
  ...
  for (const plugin of plugins) {
    const extendedPlugin = authPluginExtensions(plugin)
    ...
    const PluginView = extendedPlugin.views?.auth?.[pluginView]
    if (!PluginView) continue
  ...
  if (authView === "signIn" && !emailAndPassword?.enabled) {
    const Fallback = plugins.find(
      (plugin) => authPluginExtensions(plugin).fallbackViews?.auth?.signIn
    )

    const FallbackView = Fallback
      ? authPluginExtensions(Fallback).fallbackViews?.auth?.signIn
      : undefined
