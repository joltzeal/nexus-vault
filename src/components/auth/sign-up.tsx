...
export function SignUp({
  ...
  const captchaPlugin = plugins.find(
    (plugin) => authPluginExtensions(plugin).captchaComponent
  )
  const Captcha = captchaPlugin
    ? authPluginExtensions(captchaPlugin).captchaComponent
    : undefined
              ...
              <FieldGroup>
                ...
                <div className="flex flex-col gap-3">
                  ...
                  {plugins.flatMap((plugin) =>
                    (authPluginExtensions(plugin).authButtons ?? []).map((AuthButton, index) => (
                      <AuthButton
