...
import type {
  ...
  XPostCardData,
} from "@/features/components/resource-cards/types"
import { PreviewMedia } from "@/features/components/resource-cards/preview-media"
import { formatCompactNumber } from "@/features/components/resource-cards/resource-card-frame"
import { XUserHeader } from "@/features/components/resource-cards/x-user-header"
  ...
  return (
    <div className="flex min-w-0 flex-col gap-3">
      ...
      {tweet ? (
        ...
        <p className="mono text-xs text-fg-dim">Tweet ID: {data.tweetId}</p>
      )}
  ...
  return (
    <div className={cn("flex min-w-0 flex-col gap-3", className)}>
      <div className="flex items-center gap-3">
        <Skeleton className="size-10 shrink-0 rounded-full bg-ink-700" />
      ...
      {media && <Skeleton className="aspect-video w-full bg-ink-700" />}
    </div>
  )
}
...
export function TweetNotFound({ className }: { className?: string }) {
  return (
    <div className={cn("flex min-h-28 items-center justify-center text-sm text-fg-dim", className)}>
      无法读取这条 Tweet
    </div>
  )
}
...
function TweetBody({ tweet }: { tweet: EnrichedTweet }) {
  return (
    <p className="mono whitespace-pre-wrap break-words text-xs leading-5 text-fg-muted">
      {tweet.entities.map((entity, index) => {
        ...
        return (
          <a
            className="text-jade transition-colors hover:text-jade-bright"
            href={entity.href}
...
function TweetMetrics({
  ...
  return (
    <dl className="flex min-w-0 flex-wrap items-center gap-x-4 gap-y-1.5 text-fg-dim">
