          ...
          <div className="flex min-w-0 items-start gap-2.5">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex min-w-0 flex-wrap items-center gap-1.5">
                <h2 className="truncate text-[15px] font-semibold text-fg">{chatName}</h2>
                {data.chatType && (
                  <Badge variant="secondary">{getChatTypeLabel(data.chatType)}</Badge>
                )}
              </div>
              <p className="mono truncate text-[11px] text-fg-dim">
          ...
          {authorName && !authorIsChat && (
            <div className="flex min-w-0 items-center gap-2">
              ...
              <div className="flex min-w-0 items-baseline gap-1.5">
                <span className="truncate text-xs font-medium text-fg-muted">{authorName}</span>
                {data.authorUsername && data.authorName && (
                  <span className="mono truncate text-[10.5px] text-fg-dim">
                ...
                {data.reactions && data.reactions.length > 0 && (
                  <div className="flex min-w-0 flex-wrap gap-1.5">
                    {data.reactions.map((reaction) => (
                      <Badge key={reaction.emoji} variant="outline">
...
function TelegramMetric({
  ...
  return (
    <span className="inline-flex min-w-9 flex-col items-center gap-0.5 text-[11px] text-fg-dim" title={label}>
