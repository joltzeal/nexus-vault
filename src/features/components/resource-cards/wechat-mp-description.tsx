...
export function WechatMpDescription({
  ...
  return (
    <div
      className={cn(
        "group/wechat-html relative min-w-0 overflow-hidden rounded-input border border-line-soft bg-ink-900/45 transition-colors hover:border-line",
        className
      )}
    ...
    >
      <div className="flex items-center justify-between gap-2 border-b border-line-soft px-2.5 py-1.5">
        <span className="mono min-w-0 truncate text-[10px] uppercase text-fg-dim">
          Article HTML
        </span>
        <Button
          className="size-5 shrink-0 rounded-sm text-fg-dim opacity-70 transition group-hover/wechat-html:opacity-100 hover:text-jade [&_svg]:size-3"
          onClick={(event) => {
      ...
      />
      {!open && (
        <div className="pointer-events-none absolute inset-x-0 bottom-0 h-16 bg-gradient-to-t from-ink-900 via-ink-900/85 to-transparent" />
      )}
      <div className="relative border-t border-line-soft bg-ink-900/80 px-2.5 py-1.5">
        <Button
          className="h-6 w-full justify-center rounded-sm text-[11px] text-fg-dim hover:bg-ink-800 hover:text-jade"
          onClick={(event) => {
            event.stopPropagation()
            setOpen((value) => !value)
          }}
