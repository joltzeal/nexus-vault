    ...
    try {
      ...
      if (isTransientNetworkError(error)) {
        const retryable = new RetryableMetadataError(
          `GitHub request failed temporarily: ${getErrorMessage(error)}`,
        )
      ...
      return {
        ...
        errorMessage: error instanceof Error ? error.message : "GitHub request failed.",
      }
    }
  },
}
...
function getIdentifiers(parsed: NonNullable<ReturnType<typeof parseGitHubLink>>) {
